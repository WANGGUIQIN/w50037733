...
_TASK_PROMPTS = {
    ...
    "planning": (
        ...
        'task: "{text}". Choose operation primitives from '
        '{{reach, grasp, transport, place, push, pull, insert, pour, rotate, '
        'release, flip, wipe}}.\n\n'
        'Output EXACTLY this compact text format. No markdown, no JSON, no '
        'bullets, no bold. Each field on its own line, exactly as shown:\n'
        'Scene: <obj1>, <obj2>, ...\n'
        'Step 1: <action>(<target>)\n'
        '  affordance_hint: <natural-language functional part>\n'
        '  affordance: [u, v]\n'
        '  approach: [x, y, z]\n'
        '  contact: <pred>(<args>) [<role>]; ...\n'
        '  spatial: <pred>(<args>) [<role>]; ...\n'
        '  done_when: <expression>\n'
        'Step 2: <action>(<target> -> <destination>)\n'
        '  ...\n\n'
        'Rules:\n'
        '  - affordance_hint MUST appear before affordance on its own line.\n'
        '  - [u, v] are normalized floats in [0.0, 1.0]; NEVER 0-1000 pixels.\n'
        '  - For transport/place/insert/pour: use "<target> -> <destination>" '
        'in the step header, and the affordance points at the DESTINATION.\n'
        '  - For grasp/pick/lift/pull on an object with a handle: hint = '
        '"the handle of the <obj>". For objects without a handle (block, '
        'towel, cloth, fruit, package, butter, sponge, paper): hint = '
        '"the <obj>".\n'
        '  - For place/transport: hint = "an empty area on the <destination>".\n'
        '  - Constraints categories: contact / spatial / pose / direction / '
        'safety. Each predicate has the form pred(args) [role]; omit a '
        'category line if no constraint applies.\n\n'
        'Example (follow the FORMAT, not the values):\n'
        'Scene: red_mug, blue_plate, table\n'
        'Step 1: reach(red_mug)\n'
        '  affordance_hint: the red mug\n'
        ...
        '  affordance_hint: the handle of the red mug\n'
        '  affordance: [0.32, 0.51]\n'
        '  approach: [0.00, 0.00, -1.00]\n'
        ...
        '  affordance_hint: an empty area on the blue plate\n'
        ...
        'Now produce the plan for the actual scene. Every step MUST include '
        'affordance_hint and affordance on separate lines.'
    ),
    "general": "{text}",
}
